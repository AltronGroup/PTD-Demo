...
public class NfcActivity extends AppCompatActivity  implements View.OnClickListener {
    ...
    private void initView() {
        ...
        bOpen.setOnClickListener((View.OnClickListener) this);
        bCheck.setOnClickListener((View.OnClickListener) this);
        bAuthen.setOnClickListener((View.OnClickListener) this);
        bRead.setOnClickListener((View.OnClickListener) this);
        bWrite.setOnClickListener((View.OnClickListener) this);
        bApdu.setOnClickListener((View.OnClickListener) this);
        ...
        if (view == bOpen) {
            ...
            exec.execute(new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    sak[0] = (byte) SAK;
                    byte SN[] = new byte[10];
                    scanCard = piccManager.request(CardType, Atq);
                    if (scanCard > 0) {
            ...
            exec.execute(new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (!key.equals("") && isHexAnd16Byte(key)) {
                        ...
                        ret = piccManager.m1_keyAuth(0, blkNo, 6, keyBuf, SNLen, SN);
                    }
                    if (ret == -1) {
            ...
            exec.execute(new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    int result = piccManager.m1_readBlock(blkNo, pReadBuf);
                    if (result == -1) {
            ...
            exec.execute(new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    int result = piccManager.m1_writeBlock(blkNo, writeData.length,
                            writeData);
                    if (result == 0) {
            ...
            exec.execute(new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (!reception.equals("") && isHexAnd16Byte(reception)) {
                        ...
                        result = piccManager.apduTransmit(EMV_APDU, EMV_APDU.length, readBuf, sw);
                    }
                    if (result == -1) {
